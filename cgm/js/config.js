...
(function($, c) {
  ...
  function encodeLayout() {
    ...
    return {
      elements: elements,
      batteryLoc: $('#batteryLoc').value,
      timeAlign: $('#timeAlign').value,
    };
  }
  ...
  function populateValues(current) {
    $('#ns-url').value = current['nightscout_url'] || '';

    if (current.mmol === true) {
      $('#units-mmol').className += ' active';
    } else {
      $('#units-mgdl').className += ' active';
    }
    ...
    $('#hGridlines').value = current['hGridlines'];
	
	$('#colorEnabled').value = current['colorEnabled'];
	$('#colorAbove').value = current['colorAboveN'];
	$('#colorWithin').value = current['colorWithinN'];
	$('#colorBelow').value = current['colorBelowN'];
	
    $('#statusContent').value = current['statusContent'];
    $('#statusText').value = current['statusText'] || '';
    $('#statusUrl').value = current['statusUrl'] || '';
    $('#statusJsonUrl').value = current['statusJsonUrl'] || '';
  ...
  function buildConfig() {
    ...
    var mmol = $('#units-mgdl').className.indexOf('active') === -1;
    var out = {
      version: c.VERSION,
      mmol: mmol,
      nightscout_url: $('#ns-url').value.replace(/\/$/, ''),
      hGridlines: tryParseInt($('#hGridlines').value),
	  colorEnabled: $('[name=colorEnabled]').is(':checked'),
	  colorAbove: $('[name=colorAbove]').val(),
	  colorWithin: $('[name=colorWithin]').val(),
	  colorBelow: $('[name=colorBelow]').val(),
      statusContent: $('#statusContent').val(),
      statusText: $('#statusText').value,
      statusUrl: $('#statusUrl').value,
      statusJsonUrl: $('#statusJsonUrl').value,
